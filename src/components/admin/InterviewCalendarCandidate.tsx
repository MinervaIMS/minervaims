...
export default function InterviewCalendarCandidate() {
  ...
  const datesWithSlots = useMemo(() => new Set(available.map((s) => s.slot_date)), [available]);
  const daySlots = useMemo(
    () => available.filter((s) => selectedDate && isSameDay(parseISO(s.slot_date), selectedDate)),
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>Your current slot will be released and you can pick another available time.</AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardContent className="pt-6">
              <Calendar
                ...
                disabled={(date) => !datesWithSlots.has(format(date, 'yyyy-MM-dd'))}
              ...
              <div className="mt-4 text-xs font-body text-muted-foreground">
                <span className="underline underline-offset-4 text-accent font-semibold">Underlined</span> = days with available slots
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-6">
              ...
              {daySlots.length === 0 ? (
                <p className="font-body text-sm text-muted-foreground py-8 text-center">No slots available on this date. Try another highlighted day.</p>
              ) : (
