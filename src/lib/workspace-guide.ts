...
export const GUIDE: GuideEntry[] = [
  ...
  },
  {
    ...
    view: ['See every application in the semester, whatever your division.', 'Open candidate profiles, preview and download documents.', 'Read which division is evaluating each candidate, and filter the register by it.', 'Step from one candidate to the next with the arrows at the top of the candidate window.', 'Team Leaders and Portfolio Managers can also add notes, without changing statuses.'],
    manage: ['Change candidate statuses for the candidates your division is assessing (some statuses email the candidate automatically) and add notes.', 'Move ANY candidate to a different division, including one another division is assessing.'],
    ...
    topics: [
      { id: 'status', title: 'Candidate status dropdown', body: 'A candidacy only moves FORWARD: the dropdown offers only stages later than the current one, and the server refuses any attempt to move a candidate back. Statuses marked "sends an email / action" also email the candidate, for example an interview invitation, and a confirmation dialog always appears first; those emails cannot be recalled. Offer outcomes - offer accepted, offer declined, joined - are set by the offer process in New Joiners and by the applicant\u2019s own reply, so they cannot be chosen here. "Accepted" is an INTERNAL decision and is not visible to the candidate: they still see their outcome as pending until the President sends the offers.', requires: 'manage' },
      { id: 'internal-statuses', title: 'The four the association keeps to itself', body: 'On hold pre interview, On hold post interview, Plausible offer and To be rejected record a pause, or a decision taken and not yet acted on. NONE OF THEM SENDS ANYTHING: no email, no confirmation dialog, nothing unlocked, and the candidate\u2019s own status page reads each as the stage they were already at, so setting one tells them nothing. They exist so a division can finish making up its mind inside the workspace instead of in a spreadsheet. '
        + 'THERE ARE TWO HOLDS, one on each side of the interview, and which you pick matters, because a status only ever moves forward. "On hold pre interview" sits between "Under review" and "To be invited", so everything from the invitation onwards is still ahead of a candidate parked on it: use it for the commonest pause, a CV read that is neither an obvious invitation nor an obvious refusal. "On hold post interview" sits after "Interview completed", with Plausible offer and To be rejected, for somebody you have now met. '
        + 'NONE OF THE FOUR IS A DEAD END: Accepted and Rejected are reachable from every one of them, so "To be rejected" can still become an offer if the round turns out differently. Rejecting from any of them sends the ordinary rejection email, and the right one: post-interview for somebody who was interviewed, pre-interview for somebody who was not.', requires: 'manage' },
      { id: 'interview-changes', title: 'Changing an interview slot', body: 'A candidate books once and may change that booking once. The second change is refused, by the server as well as by their page, and the Cancel button disappears when it is spent. Every change hands a time back to the division that opened it and takes it from whoever books next, so a candidacy that rearranges four times is rearranging everybody else\u2019s day. Both the booking and any change are written into the candidate\u2019s notes automatically, so the history is where you are already reading. Moving a candidate to another division resets the count: they have not used up a change of THAT division\u2019s time.' },
      {
        ...
        body: 'The first column of the register, and the only one that records a decision rather than a request. First choice and second choice are what the applicant asked for; "Evaluated for" is which division is actually assessing them, and it is what every email they receive names, which interview calendar they can book, and which division sends them an offer. It starts as their first choice, so for most candidates it simply reads the same and nothing about the process changes. '
          + 'Roles that can change a candidate\'s status can change it, to any of the five research divisions or to the joint Media & Communication and Operations intake, which is wider than the form: an applicant ranks the research divisions or applies once to the joint intake, while an examiner may conclude that somebody belongs in a division nobody named. Media & Communication and Operations are one intake throughout, with one interview calendar run by both of their Heads; which of the two a joiner goes to is decided by the role they are offered. '
          + 'THEY CAN CHANGE IT FOR ANY CANDIDATE, not only the ones their own division is assessing. Reassignment exists precisely because a candidate is sitting in the wrong place, and the person who notices is usually the head of the division they belong in rather than the head of the one holding them. Advancing, inviting and rejecting stay with the assessing division; claiming a candidate for yours does not. '
      ...
      { id: 'priority', title: 'Priority', body: 'A marker for the candidates to be looked at first. It is off for everybody until somebody turns it on, from inside the candidate\u2019s own window, and it then shows as a red mark in its own column beside the LinkedIn one, for every reviewer. It changes nothing else: no status moves and nothing is sent. The candidate cannot see it: it is held apart from their application, where they have no access to it, precisely because it is a reviewers\u2019 judgement about them. Setting it is reserved for the roles that manage this page, the President, the Vice President, the Head of Asset Management and the Heads of Division, for any candidate they can see, including one another division is assessing.' },
      { id: 'withdrawn', title: 'Candidates who have withdrawn', body: 'An applicant can stop their own candidacy from their workspace at any point while it is running. When they do, the application stays exactly where it is in this register, readable in full by every interviewer and examiner, with its own status, "Withdrawn by candidate", and the date they withdrew shown inside their window. Nothing is deleted and nothing is hidden. What changes is that the candidacy is closed: it cannot be advanced, invited, rejected, offered or moved to another division, and any interview slot they were holding has already gone back to the division that opened it. It is not a rejection and not a declined offer, and it is recorded as neither. Withdrawing is the candidate\'s act alone; no role in the workspace can do it on their behalf, and none can reverse it.' },
      ...
      { id: 'scope', title: 'Which candidates you see, and what you may do with them', body: 'EVERYBODY WHO CAN OPEN THIS PAGE SEES EVERY APPLICATION in the semester: the President, the Vice President and the Head of Asset Management, the Heads of Division, and the Team Leaders and Portfolio Managers. A selection round is judged as a whole or not at all, and the Evaluated for filter narrows the register to your own division in one press. '
        + 'WHAT YOU MAY DO WITH THEM IS A DIFFERENT QUESTION, and it has two answers. Team Leaders and Portfolio Managers read and add notes, and nothing else. The roles that manage this page - the President and Admin, the Vice President, the Head of Asset Management and the Heads of Division - can MOVE ANY CANDIDATE to another division, because that is exactly what reassignment is for. Advancing a candidate, inviting them to interview and rejecting them stay with the division that is assessing them, since those acts speak to the candidate in that division\'s name and an invitation opens that division\'s calendar. On somebody else\'s candidate you will therefore see the Evaluated for control but not the status one, and the window says so.' },
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'bulk', title: 'Opening several slots at once', body: 'Smart planning creates a run of back-to-back slots between a start and an end time on one day, all carrying the same meeting link. Ideal for a full interview afternoon. Slots last 30 minutes unless you choose 15 in "Slot length"; candidates see the start and end of their slot, and their booking confirmation states its length. A slot that would overlap one you already have that day in the same calendar is left out, so running the same afternoon again, or cutting it into 15 minutes after opening it in half hours, never creates two interviews at once.', requires: 'manage' },
  ...
  },
  {
    ...
    manage: ['Send or resend offers; the candidate has three days to accept from their workspace.', 'Withdraw an offer that is ready or still awaiting a reply.'],
    warnings: ['Sending an offer emails the candidate immediately and cannot be reversed.', 'When the candidate accepts, their account becomes a member automatically.', 'Withdrawing an offer closes the candidacy and emails the candidate at once.'],
    topics: [
      ...
      { id: 'order', title: 'Order, filters and moving between candidates', body: 'Candidates are listed in the order they were selected for an offer, first selected first; the Selected column gives the date. The search box and the filters in the column headers (Division, Year, Programme, Offer) work as in Candidate Screening, and "Clear filters" removes them all. In a candidate\'s window, the arrows in the top right corner move to the previous or next candidate in the list as it is filtered.' },
      { id: 'withdraw', title: 'Withdrawing an offer', body: 'An offer can be withdrawn while it is Ready to offer or Offer sent and awaiting a reply, from the Withdraw button in the row or in the candidate\'s window. The candidacy closes as not selected. If the offer had been sent, the candidate receives the "Offer withdrawn" email and can no longer accept it from the workspace; if it had never been sent, they receive the usual post-interview rejection and do not learn of the offer. An offer already accepted cannot be withdrawn here. A withdrawn offer stays on the page, marked as such, and a new offer can still be sent.', requires: 'manage' },
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'advisor-here', title: 'Making somebody an advisor', body: 'Advisor is one of the roles on this list. Granting it here gives the person an advisor\'s access straight away: they can read every section of the workspace and change none of it apart from their own profile, Settings stays closed to them, they leave the membership fee (any unpaid fee for an open collection is cleared, while a payment already made is kept), and they start hidden from the public Members page. THE ONE THING IT DOES NOT DO is add them to the alumni directory. An advisor is, by the association\'s own definition, an appointed alumnus, so where the person is also leaving the active membership, People > Members > Move to alumni is the better route: it registers them as an alumnus and appoints them advisor in a single step. Use this page when the alumni record already exists, or when only the role needs correcting.', requires: 'manage' },
      { id: 'current-role', title: 'A role this page cannot grant', body: 'Some roles are reached another way: alumni through the leave flow, candidate through an application, and admin belongs to the association account alone. When somebody holds one of those, it is shown at the top of the list as their current role and cannot be selected, so the field always states what they are today even where this page cannot set it.' },
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'view-in-practice', title: 'What View looks like on the page', body: 'On a subsection where your role has View, the controls that would change something are FADED AND CANNOT BE PRESSED: the create and add buttons, the edit and delete icons on a row, the switches that publish something, the Save at the bottom of a form. You can see what each one would do, and that it is not yours to do. Everything that only changes what you see stays exactly as it is: search, filters, sorting, downloads and exports, opening a record to read it, previewing a document, registering yourself for an event, and the help. This is a courtesy, not the lock. The lock is on the server, which refuses the same actions however the request reaches it; what the fading changes is that you are told before you fill in a form rather than after.' },
  ...
  {
    key: 'people-invites', section: 'People', label: 'Invites',
    purpose: 'Accounts for advisors and alumni, who no longer hold a university address and so cannot register themselves.',
    view: ['Read who has been invited, as what, by whom, and whether they have accepted.'],
    manage: ['Invite an advisor or an alumnus, send an invitation again, or revoke one that has not been accepted.'],
    warnings: [
    ...
    topics: [
      ...
      { id: 'states', title: 'Waiting, Accepted, Revoked', body: 'Waiting means the invitation has been sent and the link has not been used. Accepted means they have set a password and the account is live; from that point their access is managed in Settings, Users, like anybody else\u2019s, and this page only records that it happened. Revoked means somebody withdrew it before it was used: the link stops working, the access it carried is taken back, and the row stays so it is clear who invited whom and when.', requires: 'manage' },
      { id: 'resend', title: 'If nobody replies', body: 'Resend sends a fresh link to the same address and records that it went out again, with a count. It is the right answer to an invitation that was never seen; a revoked invitation cannot be resent, because revoking is a decision, so send a new one instead.', requires: 'manage' },
      { id: 'division', title: 'The division field', body: 'Optional, and a record only. It says where an alumnus belonged or which division an advisor is closest to, so the register reads sensibly later. It grants nothing and opens nothing.', requires: 'manage' },
    ],
  },
  {
    ...
    topics: [
      ...
      { id: 'withdrawing', title: 'Withdrawing your application', body: 'If you no longer wish to be considered, you can withdraw at any point while your application is running: from the moment it is received to the moment you answer an offer. The control is at the foot of this page. You are asked to sign by writing your full name, exactly as you would to accept an offer, so that it cannot happen by a stray click, and you receive an email confirming it. Withdrawing releases any interview slot you were holding and stops every further message about this round. It cannot be undone from the workspace: if you withdraw by mistake, write to the association at once. Only one application per person is accepted in each round, so withdrawing does not free you to submit a new one for the same intake; a future round is a different matter and you are welcome to apply then.' },
    ],
  },
  {
