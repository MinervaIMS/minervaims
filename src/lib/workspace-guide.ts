...
export const GUIDE: GuideEntry[] = [
  ...
  },
  {
    ...
    topics: [
      ...
      { id: 'screening-mark', title: 'Mark: To reject and Maybe', body: 'A reviewer\u2019s own shorthand, kept deliberately apart from the status. "Maybe" means a candidate you would interview if places remain; "To reject" is a decision taken but not yet acted on. NOTHING IS ATTACHED TO EITHER: no email is sent, no stage moves, nothing is unlocked, and the candidate never sees it. Set it, change it and clear it in any order, at any stage. It exists because a mark that went in the status column could not work: the status only ever moves forward, so a candidate marked "Maybe" as a status could never afterwards be invited to interview, which is the one thing the mark is for. It is filterable from the register, so "show me the maybes" is one press.' },
