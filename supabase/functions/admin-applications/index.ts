...
Deno.serve(audited('admin-applications', async (req, audit) => {
  ...
  try {
    ...
    //
    // The candidate is notified of the new evaluation division; a later
    // interview invitation is still sent by `update-status` in the usual way.
    // =====================================================================
    ...
    if (action === 'set-evaluation-division') {
      ...
      const { error } = await supabase.from('applications')
        .update({
          ...
          // stage that followed it belonged to the old division. The count
          // of slots taken goes with them: a candidate handed to another
          // division has not used up a change of THEIR time.
          interview_division: null,
          interview_bookings_made: 0,
