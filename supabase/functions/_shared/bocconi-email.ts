...
// got subtly wrong.
//
// THIS IS THE SERVER'S COPY. The browser's is src/lib/bocconi-email.ts,
// and it cannot be imported from here: an edge function does not share a
// module graph with the site. The two must be changed together, and the
// test suite compares them.
//
// THE BROWSER'S COPY IS A COURTESY, THIS ONE IS THE RULE. The form
// checks the address before it creates anything, so that a mistyped
// domain is a sentence under a field rather than a rejected application
// ten seconds later. That check runs on the applicant's machine and can
// simply be skipped, so nothing is trusted to it.
// =====================================================================
