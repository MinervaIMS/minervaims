-- =====================================================================
-- Our History — the timeline on /about, editable from the workspace.
-- ---------------------------------------------------------------------
-- One row per YEAR. The section is a spine of years, and each year either
-- carries a key event or stands as a quiet marker keeping the rail
-- continuous, so "one event per year" is not a convention to remember: it
-- is the primary key.
--
-- Three rules are enforced here rather than in the form alone, because a
-- register that can be written from more than one place has to hold its
-- own shape:
--   * one event per year        -> year is the primary key
--   * never a future year       -> CHECK against the current year
--   * a title and a description -> NOT NULL, and non-empty
--
-- A milestone may carry one piece of media: a published report's cover, a
-- fixed number (the "100+" of the alumni card), or an uploaded image.
-- =====================================================================
...
CREATE TABLE IF NOT EXISTS public.history_events (
  ...
  /** Optional destination for the milestone's title link. */
  href           text,
  media_kind     text NOT NULL DEFAULT 'none',
  /** media_kind = 'report': the archive file whose cover is shown. */
  report_file_id uuid REFERENCES public.archive_files(id) ON DELETE SET NULL,
  /** media_kind = 'number': a fixed figure and its caption. */
  number_value   integer,
  number_label   text,
  /** media_kind = 'image': an uploaded photograph. */
  image_url      text,
  image_alt      text,
  /** A quiet year: kept on the rail, drawn small, with no card. */
  ...
  -- A society cannot record what has not happened yet.
  CONSTRAINT history_events_not_future
    CHECK (year <= EXTRACT(YEAR FROM now())::int),
  CONSTRAINT history_events_year_sane
    CHECK (year >= 2019),
  -- An active year always says something; a quiet year never does.
...
-- The timeline is a public page: everybody reads it, nobody writes it from
-- the client. Every change goes through the admin-history edge function,
-- which checks the role first.
...
-- Seed the timeline the page already shows, so nothing disappears the
-- moment it starts reading from this table. Quiet years are seeded too, so
-- the rail keeps its shape and an editor can simply fill one in.
...
-- The report covers are resolved by DESCRIPTION rather than by id, exactly
-- as the page did before, so re-uploading the archive does not break them.
