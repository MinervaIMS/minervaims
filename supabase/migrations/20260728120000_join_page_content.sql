-- ═════════════════════════════════════════════════════════════════════
-- /join page content — admissions FAQs and the divisions' written questions
-- ---------------------------------------------------------------------
-- Everything on /join that changes between intakes now lives in the
-- database, so a new semester never needs a code deploy:
--
--   join_faqs                  the four accordion groups on /join
--   join_written_questions     the question the Board sets per division
--
-- Application status (open/closed, semester label, deadline) already lives
-- in application_settings and is untouched here.
--
-- Both tables are publicly readable and writable only from the workspace,
-- matching the pattern used by the rest of the public content tables.
-- ═════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS public.join_faqs (
  ...
  -- One of the four groups rendered as accordion sections, in this order.
  faq_group    text NOT NULL CHECK (faq_group IN ('eligibility', 'application', 'preparing', 'membership')),
  -- Order within the group. Sparse (10, 20, 30 ...) so entries can be
  -- inserted between two existing ones without renumbering the group.
  position     integer NOT NULL DEFAULT 0,
  question     text NOT NULL,
  answer       text NOT NULL,
  -- Optional routed link rendered after the answer, e.g. "Read The Statute"
  -- to /statute. Kept as two columns rather than markup inside `answer`, so
  -- the workspace can validate the destination and the page never renders
  -- raw HTML from the database.
  link_label   text,
  link_to      text,
...
-- 2. WRITTEN QUESTIONS
-- ─────────────────────────────────────────────────────────────────────
-- One row per core division, created empty. The Board of Directors sets
-- `question` ahead of each intake; until then /join renders its designed
-- empty state. A NULL or blank question is the normal resting state
-- between intakes, not an error.
...
-- 3. FAQ SEED
-- ─────────────────────────────────────────────────────────────────────
-- Dollar-quoted so apostrophes in the copy need no escaping and the text
-- stays byte-identical to the approved wording.
