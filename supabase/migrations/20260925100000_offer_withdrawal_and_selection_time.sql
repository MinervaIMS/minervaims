-- =====================================================================
-- OFFERS: WHEN A CANDIDATE WAS SELECTED, AND WITHDRAWING AN OFFER.
-- ---------------------------------------------------------------------
-- 1. applications.selected_at: the moment a candidacy became "accepted",
--    that is, when the candidate was selected to receive an offer. The
--    Offers page lists candidates in that order. A trigger sets it the
--    first time the status becomes "accepted", whoever makes the change;
--    it is never moved afterwards (an offer re-sent after a decline or a
--    withdrawal keeps the original selection time).
--
--    Existing candidates are backfilled from the best evidence there is:
--    the last successful status change recorded for them in the activity
--    log before their offer was sent (the change to "accepted", since a
--    candidacy only moves forward); failing that, the time the offer was
--    sent; failing that, the last update of the application.
--
-- 2. applications.offer_withdrawn_at / offer_withdrawn_by: set by the new
--    "withdraw-offer" action of admin-applications, which also moves the
--    candidacy to "rejected" and emails the candidate. The offer deadline
--    job only looks at "accepted" rows, so a withdrawn offer gets no
--    reminder and no expiry email.
--
-- 3. The "Offer withdrawn" email, seeded exactly as the Auto emails page
--    stores it (its real home is _shared/transactional-emails.ts), because
--    the send path sends nothing for a key without a row.
--
-- Nothing existing changes: new columns are empty until used, and the
-- trigger only fills selected_at when it is empty. Idempotent.
-- =====================================================================
...
-- Backfill for candidates already selected.
