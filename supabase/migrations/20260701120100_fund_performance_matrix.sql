-- =====================================================================
-- Fund performance matrix (Reports batch)
-- ---------------------------------------------------------------------
-- The public Long-Short and Multi-Asset fund pages render a year x month
-- performance matrix (ITD, twelve monthly returns, YTD, Vol, Sharpe). The
-- workspace must edit exactly this so that updating a month in the
-- workspace shows immediately on the public fund page. We store the table
-- as one row per (fund, year) with the twelve monthly returns in a jsonb
-- array. Values are kept as text to preserve the exact display formatting
-- (e.g. "+0.9%", "1.68", "" for an empty month).
-- The table is public-readable (data connection only, no restyling) and
-- writable only through the admin-funds edge function (service role).
-- =====================================================================
...
-- Seed the existing public data so the fund pages keep their current tables.
